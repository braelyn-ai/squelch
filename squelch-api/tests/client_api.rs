...
async fn a_bcc_reaches_the_header_and_the_ledger_counts_it() {
    ...
    let detail = store
        ...
        .and_then(|a| a.detail);
    assert_eq!(detail.as_deref(), Some("ok:cc:1:bcc:2"));
    assert!(
...
#[tokio::test]
async fn a_bcc_carrying_a_smuggled_header_line_is_refused_outright() {
    // The composer's fields are typed by a human, but the wire is a wire. A
    // newline in the one list nobody downstream can read is the least visible
    // way there is to add a recipient.
    let (base, handle) = mock_gmail_seq(vec![(200, group_metadata_body())]).await;
    let Harness { app, store, acct } = app_with_writes(base, |store, acct| {
        seed_one_signal(store, acct, "gmail-parent", "thread-77", "Lunch?");
    });
