...
impl<S: Store + 'static, C: CredentialStore + 'static + ?Sized> SyncEngine<S, C> {
    ...
    async fn catch_up(&self) -> Result<()> {
        ...
        // the `?` paths below, which is what `_guard` is for. A catch-up is this
        // loop's longest single call by orders of magnitude and it used to emit
        // nothing at all while it ran: no log, no counter, no freshness stamp,
        // because `poll_once` had not returned yet. Worse, most of its work is
        // upserting mail already stored, so the message count and the database
        // size sit still as well. On 2026-08-26 a tenant's mailbox spent half an
        // hour in here and the only honest thing anybody could say from outside
        // was that it was indistinguishable from wedged.
