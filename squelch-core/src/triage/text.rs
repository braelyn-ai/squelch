//! Triage-local text helpers: the neutralizer every fenced prompt puts
//! UNTRUSTED text through, and the constructors each detector builds its static
//! regex battery from. Truncation itself is [`crate::text`].

pub(crate) use crate::text::truncate_chars;
use regex::Regex;
...
/// Compile a case-insensitive static detector pattern. Every caller passes a
/// hard-coded literal, so a bad pattern is a build bug, not input.
