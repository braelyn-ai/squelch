...
Search that finds the mail you mean, spam you can see, and login codes that arrive on time.
...
- Groups no longer report a delivery shortfall on sends that went out one person at a time
- Pages have a title and a subtitle instead of a title with a dash in it
- Rules, audit and groups fill the window again instead of stopping halfway down
...
- The home screen icon shows how many things are due today, even while the app is closed. If you allowed notifications before this release, turn Badges on in Settings to see it
...
- A message skipped for having no body is extracted once the body arrives
