...
// the only webview in the app. Five independent sandbox layers (page JS off,
// injected CSP, navigation refused, non-persistent store, images proxied
// through our own fetch path): see docs/SECURITY.md. The frame is sized to its
// measured content and never scrolls itself; the thread column is the one scroll.
