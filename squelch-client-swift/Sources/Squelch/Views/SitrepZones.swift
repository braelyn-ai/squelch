                ...
                VStack(spacing: 1) {
                    ForEach(rows) { r in
                        Button {
                            ...
                            HStack(spacing: 7) {
                                Text(SenderID.displayName(r.senderString))
                                    .font(Typo.rowSub)
                                    .foregroundStyle(Palette.inkDim)
                                    .lineLimit(1)
                                    .frame(maxWidth: .infinity, alignment: .leading)
...
struct NewslettersZone: View {
    ...
    var body: some View {
        // Hidden entirely when there are no newsletters (and while loading).
        Group {
    ...
    private func load() async {
        do {
            async let updates = APIClient.shared.getUpdates(
                UpdatesParams(tier: .noise, limit: Self.fetchLimit))
            ...
            newsletters = Newsletters.derive(
        ...
        Button(action: onOpen) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack(spacing: 6) {
                        Text(SenderID.displayName(newsletter.sender))
